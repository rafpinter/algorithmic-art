...
function art() {
    ...
    stroke(255);
    strokeWeight(1);
    const nValues = [1, 2, 3]; // 11
    
    for (const n of nValues) {
        for (let b = 1; b < 8; b += abstep) {
            for (let a = 1; a < 8; a += abstep) {
                for (let sx = xMin; sx <= xMax; sx += step) {
                    ...
                    if (is_inside_margin(center_x - y, px) & is_inside_margin(center_x + y, px)){
                        point(center_x - y, px);
                        // point(center_x + y, px);
                    }
                }
